...
: "${MOCK_UPSTREAM_PORT:=$((20021 + (RANDOM % 2000)))}"
: "${SHIM_PORT:=$((MOCK_UPSTREAM_PORT + 1))}"
: "${NIM_PROMPT:=Execute the requested tools and then return the combined outputs.}"
: "${POC_CODEX_SANDBOX:=danger-full-access}"
if [[ -z "${MOCK_TOOL_COMMANDS_JSON:-}" ]]; then
  MOCK_TOOL_COMMANDS_JSON='["printf shim-tool-a","printf shim-tool-b"]'
fi
: "${MOCK_TOOL_CALL_MODE:=sequential}"

STAMP="$(date +%Y%m%d-%H%M%S)"
ARTIFACT_DIR="${ARTIFACT_DIR:-$ROOT_DIR/artifacts/$STAMP-codex-shim-mock-multi-tool-call}"
...
MOCK_TOOL_COMMANDS_JSON="$MOCK_TOOL_COMMANDS_JSON" \
MOCK_TOOL_CALL_MODE="$MOCK_TOOL_CALL_MODE" \
